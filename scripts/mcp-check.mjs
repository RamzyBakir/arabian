...
console.log("\nPhase 1 — read-only tools against the real .arabian/");
{
  ...
  check("tools/list", names.length === 9, `${names.length} tools: ${names.join(", ")}`);

  const listed = await client.tool("arabian_list_nodes", {});
  check("arabian_list_nodes", listed.count === 18, `${listed.count} nodes`);
  ...
  check("arabian_get_graph", graph.nodes.length === 18 && graph.edges.length === 18, `${graph.nodes.length} nodes / ${graph.edges.length} edges`);
...
execFileSync("node", [join(REPO, "dist", "cli", "index.js"), "init", "--name", "mcp-check"], { cwd: tmp });
{
  ...
  const d = await client.tool("arabian_create_node", { type: "decision", title: "Pick A" });
